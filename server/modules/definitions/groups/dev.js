...
gemRelicMatrix = [];
for (let tier of [ "", "Egg", "Triangle", "Square", "Pentagon", "BetaPentagon", "AlphaPentagon" ]) {
    let row = [];
...
for (let tier = 0; tier < 6; tier++) {
    let row = [];
    for (let poly of [ "Egg", "Triangle", "Square", "Pentagon", "Hexagon" ]) {
        let column = [];
    ...
    GUNS: [
        {
            ...
            PROPERTIES: {
                SHOOT_SETTINGS: combineStats([g.ultimatum]),
                TYPE: "bullet",
            },
        }
    ],
